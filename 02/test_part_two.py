from part_two import Color, Game, GameRound, read_game, sum_of_power
...
def test_sum_of_possible_game_ids():
    ...
    assert sum_of_power(games=games) == 5


def test_power_of_maximum_colors():
    game = Game(
        round=4,
        game_rounds=[
            GameRound(store={Color.red: 5, Color.green: 4, Color.blue: 9}),
            GameRound(store={Color.red: 1, Color.green: 5, Color.blue: 7}),
        ],
    )
    assert game.power_of_maximum_colors() == 5 * 5 * 9
