...
def sum_of_possible_game_ids(games: typing.List[Game]) -> int:
    return sum([game.round for game in games if game.is_win])
...
def main():
    input_lines = read_input_lines("./part_one_input")
    game_rounds = read_games(input_lines=input_lines)
    result = sum_of_possible_game_ids(games=game_rounds)
